...
$get_location_sql = "SELECT minimum_time FROM m02_location WHERE location_id= ?";
...
mysqli_stmt_bind_result($location_stmt, $location_min_time);
mysqli_close($location_stmt);

while(mysqli_stmt_fetch($location_stmt)) {
    $location[] = array('mintime'=>$location_min_time);
}
